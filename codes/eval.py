...
p.add_argument('--root_path', type=str, default='./out/_HOLONET_ASM', help='Directory where test phases are being stored.')
...
prop_dists = (7*cm, 7*cm, 7*cm)
wavelengths = (680*nm, 520*nm, 450*nm)  # wavelength of each color
...
_,F = filter2d(torch.complex(torch.empty(1,1,*slm_res),torch.empty(1,1,*slm_res)).to(device),None,True)
...
for k, target in enumerate(image_loader):
    ...
    for c in chs:
        ...
        if slm_res[0] > 1088:
            slm_field = filter2d(slm_field,F)
    ...
    imageio.imwrite(os.path.join("amp", f'{target_idx}_rgb.png'), (recon_srgb * np.iinfo(np.uint8).max).round().astype(np.uint8))
...
print("average")
for domain in ['amp', 'lin', 'srgb']:
        psnrv = sum(psnrs[domain])/len(psnrs[domain])
        ssimv = sum(ssims[domain])/len(ssims[domain])
        print(f'PSNR({domain}): {psnrv},  SSIM({domain}): {ssimv:.4f}, ')
