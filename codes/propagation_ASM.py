...
def propagation_ASM(u_in, feature_size, wavelength, z, linear_conv=True,
    ...
    u_in = torch.fft.fftn(utils.ifftshift(u_in), dim=(-2, -1), norm='ortho')

    u_in = H * u_in

    u_in = utils.fftshift(torch.fft.ifftn(u_in, dim=(-2, -1), norm='ortho'))

    if linear_conv:
        # return utils.crop_image(u_out, input_resolution) # using stacked version
        return utils.crop_image(u_in, input_resolution, pytorch=True, stacked_complex=False)  # using complex tensor
    else:
        return u_in
...
#         # create tensor & upload to device (GPU)
#         H_exp = torch.tensor(HH, dtype=dtype).to(u_in.device)#.repeat((channel,1,1))

#         ###
#         # here one may iterate over multiple distances, once H_exp is uploaded on GPU

#         # reshape tensor and multiply
#         H_exp = torch.reshape(H_exp, (1, 1, *H_exp.size()))#(1, *H_exp.size()))#
...
if __name__ == "__main__":
    a = torch.rand(5,4,10,10).type(torch.cfloat)

    b = propagation_ASM(a,(1,1),1,1)
    c = []
    for i in range(4):
        print(a[:,i,:,:].unsqueeze(1).size())
        c.append(propagation_ASM(a[:,i,:,:].unsqueeze(1),(1,1),1,1))
    c = torch.cat(c,dim=1)
    # print(c.size())
    print((b-c))
    pass
