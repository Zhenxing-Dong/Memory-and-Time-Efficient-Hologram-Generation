...
def filter2d(slm_field,circ_filter = None,return_filter = False,dtype = torch.float32):
    dtype = slm_field.dtype
    H,W = slm_field.shape[-2:]
    if circ_filter is None:
        circ_filter = np_circ_filter(1, 1, H, W, H//2)
        circ_filter = torch.tensor(circ_filter).to(torch.float32).to(slm_field.device)
    slm_field = tfft.fftshift(tfft.fftn(slm_field, dim=(-2, -1), norm='ortho'), (-2, -1))
    slm_field = slm_field * circ_filter
    slm_field = tfft.ifftn(tfft.ifftshift(slm_field, (-2, -1)), dim=(-2, -1), norm='ortho')
    if return_filter:
        return slm_field,circ_filter
    else:
        return slm_field 
